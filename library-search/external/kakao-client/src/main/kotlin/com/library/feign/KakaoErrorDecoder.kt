...
class KakaoErrorDecoder(
    ...
    val log = LoggerFactory.getLogger(KakaoErrorDecoder::class.java)

    override fun decode(methodKey: String, response: Response): Exception {
        try {
            val body = extractBody(response)
            val errorResponse = jsonMapper.readValue(body, KakaoErrorResponse::class.java)
            throw ApplicationException(errorResponse.message, ErrorType.EXTERNAL_API_ERROR, HttpStatus.valueOf(response.status()))
        } catch (e: IOException) {
