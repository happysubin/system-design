...
@Transactional
class PartialCancellationApiTest(
    @Autowired private val mockMvc: MockMvc,
    @Autowired private val entityManager: EntityManager,
) {
    @Test
    fun `부분취소 API는 대상 승인에 취소를 반영하고 요약 응답을 반환한다`() {
        val authorization = prepareAuthorization()

        mockMvc.perform(
            post("/api/payments/authorizations/${authorization.id}/partial-cancel")
            ...
            .andExpect(jsonPath("$.authorizationStatus").value("PARTIALLY_CANCELED"))
            .andExpect(jsonPath("$.ledgerEntryType").value("CANCELLED"))
            .andExpect(jsonPath("$.ledgerEntryCount").value(2))
            .andExpect(jsonPath("$.cancelAmount").value(5000))
    }

    @Test
    fun `부분취소 API는 기존 환불 이력으로 소진된 seller에 zero ledger를 만들지 않는다`() {
        ...
        mockMvc.perform(
            ...
            .andExpect(jsonPath("$.cancelAmount").value(1))
    }

    @Test
    fun `부분취소 API는 멀티 allocation 주문에서 주문 상태를 전체 승인 기준으로 다시 계산한다`() {
        val targetAuthorization = prepareMultiAllocationAuthorization(pgTransactionId = "card-auth-api-multi-cancel-1")

        mockMvc.perform(
            post("/api/payments/authorizations/${targetAuthorization.id}/partial-cancel")
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    """
                    {
            ...
            .andExpect(jsonPath("$.authorizationStatus").value("CANCELED"))
            .andExpect(jsonPath("$.ledgerEntryCount").value(1))
    ...
    private fun prepareAuthorization(amountA: Long = 6_000L, amountB: Long = 4_000L): Authorization {
        val order = PaymentOrder(
            ...
            merchantOrderId = "order-api-cancel-001",
        ...
        val authorization = Authorization(
            ...
            pgTransactionId = "card-auth-api-001",
            approvalCode = "card-appr-api-001",
        )
    ...
    private fun prepareMultiAllocationAuthorization(pgTransactionId: String): Authorization {
        val order = PaymentOrder(
            merchantId = "merchant-1",
            merchantOrderId = "order-api-cancel-multi-allocation",
        ...
        val firstAuthorization = Authorization(
            ...
            approvalCode = "card-appr-api-multi-cancel-1",
        )
        ...
        val secondAuthorization = Authorization(
            ...
            requestedAmount = Money(50_000L, CurrencyCode.KRW),
            approvedAmount = Money(50_000L, CurrencyCode.KRW),
            pgTransactionId = "bank-auth-api-multi-cancel-2",
            approvalCode = "bank-appr-api-multi-cancel-2",
        )
