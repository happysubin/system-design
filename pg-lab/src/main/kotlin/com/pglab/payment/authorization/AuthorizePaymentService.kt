...
class AuthorizePaymentService(
    ...
    fun authorize(command: AuthorizePaymentCommand): AuthorizePaymentResult {
        require(command.lines.isNotEmpty()) { "order lines must not be empty" }
        ...
        val orderLineTotal = command.lines.sumOf { it.lineAmount.amount }
        require(orderLineTotal == command.totalAmount.amount) { "order line sum must match total amount" }

        require(command.lines.all { it.lineAmount.currency == command.totalAmount.currency }) {
            "all order line currencies must match total amount currency"
        }

        require(command.lines.map { it.lineReference }.distinct().size == command.lines.size) {
            "order line references must be unique"
        }

        val lineRequestsByReference = command.lines.associateBy { it.lineReference }
        ...
        command.allocations.forEach { allocation ->
            ...
            allocation.authorizations.forEach { authorization ->
                authorization.linePortions.forEach { linePortion ->
                    require(lineRequestsByReference.containsKey(linePortion.lineReference)) {
                        "unknown order line reference: ${linePortion.lineReference}"
                    }
                    require(linePortion.amount.currency == command.totalAmount.currency) {
                        "all line portion currencies must match total amount currency"
                    }
                }

                val approvedLinePortionTotal = authorization.linePortions.sumOf { it.amount.amount }
                require(approvedLinePortionTotal == authorization.approvedAmount.amount) {
                    "authorization line portion sum must match approved amount"
                }
            }
        }

        val cumulativeLinePortionAmounts = mutableMapOf<String, Long>()
        command.allocations.forEach { allocation ->
            allocation.authorizations.forEach { authorization ->
                authorization.linePortions.forEach { linePortion ->
                    val updatedAmount = cumulativeLinePortionAmounts.getOrDefault(linePortion.lineReference, 0L) + linePortion.amount.amount
                    val orderLineAmount = requireNotNull(lineRequestsByReference[linePortion.lineReference]) {
                        "unknown order line reference: ${linePortion.lineReference}"
                    }.lineAmount.amount
                    require(updatedAmount <= orderLineAmount) {
                        "authorization line portions for ${linePortion.lineReference} must not exceed order line amount"
                    }
                    cumulativeLinePortionAmounts[linePortion.lineReference] = updatedAmount
                }
            }
        }
        ...
        val orderLinesByReference = linkedMapOf<String, PaymentOrderLine>()
        val orderLines = command.lines.map { lineRequest ->
            PaymentOrderLine(
                lineReference = lineRequest.lineReference,
                payeeId = lineRequest.payeeId,
                lineAmount = lineRequest.lineAmount,
                quantity = lineRequest.quantity,
            ).also { orderLine ->
                order.addLine(orderLine)
                orderLinesByReference[lineRequest.lineReference] = orderLine
            }
        }
        ...
        val authorizationLinePortions = mutableListOf<AuthorizationLinePortion>()
        val authorizations = command.allocations.zip(allocationModels).flatMap { (allocationRequest, allocationModel) ->
            allocationRequest.authorizations.map { authorizationRequest ->
                Authorization(
                    ...
                    authorizationRequest.linePortions.forEachIndexed { index, linePortionRequest ->
                        val orderLine = requireNotNull(orderLinesByReference[linePortionRequest.lineReference]) {
                            "unknown order line reference: ${linePortionRequest.lineReference}"
                        }
                        AuthorizationLinePortion(
                            paymentOrderLine = orderLine,
                            payeeId = orderLine.payeeId,
                            amount = linePortionRequest.amount,
                            sequence = index + 1,
                        ).also { linePortion ->
                            authorization.addLinePortion(linePortion)
                            authorizationLinePortions.add(linePortion)
                        }
                    }
                }
            }
        ...
        val result = AuthorizePaymentResult(
            ...
            authorizationLinePortions = authorizationLinePortions,
...
data class AuthorizePaymentCommand(
    ...
    val lines: List<PaymentOrderLineRequest>,
    val allocations: List<AllocationAuthorizationRequest>,
)

data class PaymentOrderLineRequest(
...
data class AuthorizationRequest(
    ...
    val linePortions: List<AuthorizationLinePortionRequest>,
)

data class AuthorizationLinePortionRequest(
    val lineReference: String,
    val amount: Money,
)
...
data class AuthorizePaymentResult(
    val order: PaymentOrder,
    val orderLines: List<PaymentOrderLine>,
    val allocations: List<PaymentAllocation>,
    val authorizations: List<Authorization>,
    val authorizationLinePortions: List<AuthorizationLinePortion>,
